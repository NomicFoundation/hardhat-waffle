let processRequest: any = undefined
...
export function skipGasCostCheck(provider: any) {
  ...
  if (curProcessRequest !== processRequest) {
    const originalProcess = provider._hardhatNetwork.provider._wrapped._wrapped._wrapped._ethModule.processRequest.bind(
    ...
    processRequest = provider._hardhatNetwork.provider._wrapped._wrapped._wrapped._ethModule
      .processRequest
  }
}
