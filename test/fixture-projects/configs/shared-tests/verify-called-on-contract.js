...
const shouldSupportCalledOnContractMatchers = () => {
  describe('Call history injection', () => {
    it("should support calledOnContractWith", async function () {
      const contractJSON = require(process.cwd() + "/artifacts/contracts/Contract.sol/Contract.json");
      ...
      expect(() => expect("inc").to.be.calledOnContractWith(contract, [8])).to.throw();
      expect(() => expect("doNothing").to.be.calledOnContractWith(contract, [7])).to.throw();
    });
    ...
    it("should support calledOnContract", async function () {
      ...
      expect(() => expect("doNothing").to.be.calledOnContract(contract)).to.throw();
    });
  });
};
